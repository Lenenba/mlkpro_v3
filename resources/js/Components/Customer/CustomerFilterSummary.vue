...
<template>
    <section class="space-y-2" aria-labelledby="customer-filter-summary-title">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 id="customer-filter-summary-title" class="sr-only">
                    {{ t('customers.filter_summary.title') }}
                </h2>
                <p class="text-sm font-semibold text-stone-700 dark:text-neutral-200" aria-live="polite" aria-atomic="true">
                    {{ t('customers.filter_summary.results', { count: matchingCount }) }}
                    <span v-if="busy" class="font-normal text-stone-500 dark:text-neutral-400">
                        · {{ t('customers.filter_summary.updating') }}
                    </span>
                </p>
                <p v-if="hasFilters" class="mt-0.5 text-xs text-stone-500 dark:text-neutral-400">
                    {{ t('customers.filter_summary.active_count', { count: activeFilters.length }) }}
                </p>
            </div>

            <div v-if="quickFilterCount > 1" class="inline-flex w-fit rounded-sm border border-stone-200 bg-stone-50 p-1 dark:border-neutral-700 dark:bg-neutral-900" role="group" :aria-label="t('customers.filter_summary.mode_label')">
                <button
                ...
                >
                    {{ t(`customers.filter_summary.modes.${mode}`) }}
                </button>
            </div>
        </div>
        ...
        <div v-if="hasFilters" class="flex flex-wrap items-center gap-2">
            ...
            >
                ...
                <button
                    ...
                    :aria-label="t('customers.filter_summary.remove', { label: filter.label })"
                    :disabled="busy"
            ...
            <button
                ...
                @click="$emit('clear')"
            >
                {{ t('customers.filter_summary.clear_all') }}
            </button>
        </div>
    </section>
</template>
