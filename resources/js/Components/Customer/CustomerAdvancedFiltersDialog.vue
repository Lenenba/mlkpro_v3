        ...
        <div class="flex h-dvh flex-col sm:h-auto sm:max-h-[calc(100vh-3rem)]">
            <header class="flex shrink-0 items-start justify-between gap-4 border-b border-stone-200 px-4 py-4 sm:px-6 dark:border-neutral-700">
                <div>
                    ...
                    <p id="customer-advanced-filters-description" class="mt-1 text-xs text-stone-500 dark:text-neutral-400">
            ...
            <div class="min-h-0 flex-1 space-y-6 overflow-y-auto px-4 py-5 sm:px-6">
                <fieldset class="space-y-3">
                    ...
                    <div class="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                        <FloatingSelect ref="firstControl" v-model="draft.status" :label="t('customers.filters.status')" :options="statusOptions" dense />
                        <FloatingSelect v-model="draft.client_type" :label="t('customers.advanced_filters.fields.client_type')" :options="clientTypeOptions" dense />
                        ...
                        <FloatingSelect v-if="showVip" v-model="draft.is_vip" :label="t('customers.advanced_filters.fields.vip')" :options="anyOptions" dense />
                        <FloatingSelect
                            ...
                            dense
                        />
                        <FloatingSelect v-if="showQuoteFilters" v-model="draft.has_quotes" :label="t('customers.filters.quotes')" :options="anyOptions" dense />
                        <FloatingSelect v-if="showJobFilters" v-model="draft.has_works" :label="t('customers.filters.jobs')" :options="anyOptions" dense />
                    </div>
                    ...
                    <div v-if="tagOptions.length">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <label
                                ...
                                :key="option.value"
                                class="inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-sm border border-stone-200 px-3 py-2 text-xs text-stone-700 dark:border-neutral-700 dark:text-neutral-200"
                            >
                                <input
                                    type="checkbox"
                                    class="rounded border-stone-300 text-green-600 focus:ring-green-500 dark:border-neutral-600 dark:bg-neutral-900"
                    ...
                    <div v-if="showPackages" class="grid gap-3 border-t border-stone-100 pt-3 md:grid-cols-2 lg:grid-cols-3 dark:border-neutral-800">
                        <FloatingSelect v-model="draft.has_active_package" :label="t('customers.filters.active_package')" :options="anyOptions" dense />
                        <FloatingSelect v-model="draft.package_status" :label="t('customers.filters.package_status')" :options="packageStatusOptions" dense />
                        ...
                        <FloatingSelect v-model="draft.package_is_recurring" :label="t('customers.filters.package_recurrence')" :options="anyOptions" dense />
                        <FloatingSelect v-model="draft.package_recurrence_status" :label="t('customers.filters.package_recurrence_status')" :options="recurrenceStatusOptions" dense />
                    </div>
                </fieldset>
                ...
                <fieldset v-if="showAppointments" class="space-y-3 border-t border-stone-200 pt-5 dark:border-neutral-700">
                    ...
                    <div class="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                        <FloatingSelect v-model="draft.has_upcoming_appointment" :label="t('customers.advanced_filters.fields.upcoming_appointment')" :options="anyOptions" dense />
                ...
                <fieldset v-if="showBilling" class="space-y-3 border-t border-stone-200 pt-5 dark:border-neutral-700">
                    ...
                    <div class="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                        <FloatingSelect v-model="draft.has_outstanding_balance" :label="t('customers.advanced_filters.fields.outstanding_balance')" :options="anyOptions" dense />
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            >
                                ...
                                >
                                {{ t(`customers.advanced_filters.payment_statuses.${status}`) }}
                            </label>
                        </div>
                    </div>
                </fieldset>
            ...
            <footer class="sticky bottom-0 flex shrink-0 flex-col-reverse gap-3 border-t border-stone-200 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6 dark:border-neutral-700 dark:bg-neutral-900">
                ...
                <div class="flex flex-wrap justify-end gap-2">
                    <button type="button" :class="crmButtonClass('secondary', 'toolbar')" @click="reset">
                        {{ t('customers.advanced_filters.reset') }}
                    </button>
                    <button type="button" :class="crmButtonClass('secondary', 'toolbar')" @click="emit('close')">
                        {{ t('customers.actions.cancel') }}
                    </button>
                    <button type="button" :class="crmButtonClass('primary', 'toolbar')" @click="apply">
