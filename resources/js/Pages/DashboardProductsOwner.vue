...
const stockSignals = computed(() => ([
    { label: `Reserve ${formatNumber(props.stats.reserved_total)}`, tone: 'sky' },
    { label: `Avaries ${formatNumber(props.stats.damaged_total)}`, tone: 'red' },
    { label: `Lots expires ${formatNumber(props.stats.expired_lots)}`, tone: 'red' },
    { label: `Lots bientot expires ${formatNumber(props.stats.expiring_lots)}`, tone: 'amber' },
]));
...
<template>
    <AuthenticatedLayout>
        <Head title="Dashboard produits" />

        <div class="space-y-5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h1 class="text-xl font-semibold text-stone-800 dark:text-neutral-100">Dashboard produits</h1>
                    <p class="text-sm text-stone-500 dark:text-neutral-400">Vue globale ventes et stock.</p>
                </div>
