...
const props = defineProps({
    ...
    customerStats: { type: Array, default: () => [] },
...
const customerOptions = computed(() => ([
    { id: '', name: t('loyalty_module.filters.all_customers') },
    ...(props.customers || []).map((customer) => ({ id: String(customer.id), name: customer.name })),
]));
...
const applyFilters = () => {
    ...
    router.get(route('loyalty.index'), filterPayload(), {
        only: ['entries', 'filters', 'stats', 'customers', 'customerStats', 'program', 'eventOptions'],
        preserveState: true,
        preserveScroll: true,
        replace: true,
        onFinish: () => {
...
<template>
    <Head :title="$t('loyalty_module.title')" />

    <AuthenticatedLayout>
        <div class="space-y-3 loyalty-enter">
            <section class="rounded-sm border border-stone-200 border-t-4 border-t-amber-500 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <h1 class="text-xl font-semibold text-stone-800 dark:text-neutral-100">{{ $t('loyalty_module.title') }}</h1>
                        <p class="text-sm text-stone-500 dark:text-neutral-400">{{ $t('loyalty_module.subtitle') }}</p>
                    </div>
                    <Link
                        :href="route('settings.loyalty.edit')"
                    ...
                    >
                        {{ $t('loyalty_module.actions.configure') }}
                    </Link>
                </div>

                <div class="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2 xl:grid-cols-6">
                    <div class="xl:col-span-2">
                        <div class="relative">
                            <div class="pointer-events-none absolute inset-y-0 start-0 z-10 flex items-center ps-3.5">
                                <svg class="size-4 text-stone-500 dark:text-neutral-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                            ...
                            <input
                                ...
                                type="text"
                                class="w-full rounded-sm border border-stone-200 bg-white py-[9px] ps-10 pe-3 text-sm text-stone-700 placeholder:text-stone-500 focus:border-amber-500 focus:ring-amber-500 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-200 dark:placeholder:text-neutral-400"
                                :placeholder="$t('loyalty_module.filters.search_placeholder')"
                            />
                        </div>
                    </div>

                    <FloatingSelect v-model="filterForm.period" :options="periodOptions" :label="$t('loyalty_module.filters.period')" />
                    <FloatingSelect v-model="filterForm.customer_id" :options="customerOptions" :label="$t('loyalty_module.filters.customer')" filterable />
                ...
                <div v-if="showAdvanced || filterForm.period === 'custom'" class="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
                    <FloatingInput v-model="filterForm.from" type="date" :label="$t('loyalty_module.filters.from')" />
                    <FloatingInput v-model="filterForm.to" type="date" :label="$t('loyalty_module.filters.to')" />
                </div>
            ...
            <section class="grid grid-cols-1 gap-4 xl:h-[calc(100vh-25.5rem)] xl:min-h-[420px] xl:grid-cols-[300px,minmax(0,1fr)]">
                <aside class="flex h-full flex-col gap-4">
                    <div class="rounded-sm border border-stone-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                        ...
                        <div class="mt-3 grid grid-cols-1 gap-2 text-sm text-stone-700 dark:text-neutral-200">
                            <div>{{ $t('loyalty_module.program.status') }}: {{ program?.is_enabled ? $t('loyalty_module.program.enabled') : $t('loyalty_module.program.disabled') }}</div>
                            <div>{{ $t('loyalty_module.program.rate') }}: {{ Number(program?.points_per_currency_unit || 0).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 4 }) }}</div>
                            <div>{{ $t('loyalty_module.program.minimum_spend') }}: {{ formatCurrency(program?.minimum_spend || 0) }}</div>
                        </div>
                    </div>
                    ...
                    <div class="flex min-h-0 flex-1 flex-col rounded-sm border border-stone-200 bg-white p-4 shadow-sm dark:border-neutral-700 dark:bg-neutral-900">
                        ...
                        <div class="mt-3 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
                            ...
                            <div v-else v-for="item in customerStats" :key="`loyalty-customer-${item.id}`" class="rounded-sm border border-stone-200 px-3 py-2 dark:border-neutral-700">
                                <div class="truncate text-sm font-medium text-stone-800 dark:text-neutral-100">{{ item.name }}</div>
                                <div class="mt-1 flex items-center justify-between text-xs">
                                    <span class="text-emerald-700 dark:text-emerald-400">+{{ formatNumber(item.points_earned) }}</span>
                                    <span class="font-semibold text-stone-700 dark:text-neutral-300">{{ formatNumber(item.balance) }}</span>
                                    <span class="text-rose-700 dark:text-rose-400">-{{ formatNumber(item.points_spent) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                >
                    ...
                    <div
                        class="min-h-0 flex-1 overflow-auto [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-stone-100 [&::-webkit-scrollbar-thumb]:bg-stone-300 dark:[&::-webkit-scrollbar-track]:bg-neutral-700 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500"
                    >
                            ...
                            <table class="min-w-full divide-y divide-stone-200 dark:divide-neutral-700">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" class="min-w-40">
                                            <div class="px-5 py-2.5 text-start text-sm font-normal text-stone-500 dark:text-neutral-500">
                                                {{ $t('loyalty_module.ledger.customer') }}
                                            </div>
                                        </th>
                                ...
                                <tbody class="divide-y divide-stone-200 dark:divide-neutral-700">
                                    <template v-if="isLoading">
                                        <tr v-for="row in 8" :key="`ledger-skeleton-${row}`">
                                    ...
                                    <template v-else>
                                        <tr v-if="!(entries.data || []).length">
                                            <td colspan="6" class="px-4 py-8 text-center text-stone-600 dark:text-neutral-300">
                                        ...
                                        <tr v-for="entry in entries.data || []" :key="entry.id">
                                            ...
                                            <td class="size-px whitespace-nowrap px-4 py-2 text-sm text-stone-600 dark:text-neutral-300">
                                                <Link v-if="entry.customer_id" :href="route('customer.show', entry.customer_id)" class="hover:text-amber-600 hover:underline dark:hover:text-amber-300">
                                                    {{ entry.customer_name }}
                                                </Link>
                                                <span v-else>{{ entry.customer_name }}</span>
                                            </td>
                        ...
                        <nav class="flex justify-end items-center gap-x-1" aria-label="Pagination">
                            ...
                            <div class="flex items-center gap-x-1">
                                <span
                                    ...
                                    aria-current="page">{{ entries.from }}</span>
                                <span
                                ...
                                <span
                                    class="min-h-[38px] flex justify-center items-center text-stone-500 py-2 px-1.5 text-sm dark:text-neutral-500">{{ entries.to }}</span>
                            </div>
