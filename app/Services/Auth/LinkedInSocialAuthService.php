...
class LinkedInSocialAuthService
{
    ...
    public function authorizationUrl(array $provider, string $state): string
    {
        ...
        if ($authorizeUrl === '' || $clientId === '' || $redirectUri === '' || $scopes === []) {
            throw ValidationException::withMessages([
                'provider' => __('ui.auth.social.provider_not_configured', ['provider' => 'LinkedIn']),
            ]);
        }
    ...
    public function authenticate(Request $request, array $provider, array $pending): array
    {
        ...
        if ($code === '') {
            throw ValidationException::withMessages([
                'provider' => __('ui.auth.social.missing_code', ['provider' => 'LinkedIn']),
            ]);
        }
        ...
        if (! $tokenResponse->successful()) {
            throw ValidationException::withMessages([
                'provider' => $this->providerMessage(
                    $tokenResponse->json(),
                    __('ui.auth.social.token_exchange_failed', ['provider' => 'LinkedIn'])
                ),
            ]);
        }
        ...
        if ($accessToken === '') {
            throw ValidationException::withMessages([
                'provider' => __('ui.auth.social.token_exchange_failed', ['provider' => 'LinkedIn']),
            ]);
        }
        ...
        if (! $profileResponse->successful()) {
            throw ValidationException::withMessages([
                'provider' => $this->providerMessage(
                    ...
                    __('ui.auth.social.profile_fetch_failed', ['provider' => 'LinkedIn'])
                ),
            ]);
        }
        ...
        if ($providerUserId === '' || $providerEmail === '') {
            throw ValidationException::withMessages([
                'provider' => __('ui.auth.social.email_not_verified', ['provider' => 'LinkedIn']),
            ]);
        }
        ...
        return [
            'profile' => [
                ...
                'provider_email_verified' => $this->resolveProviderEmailVerified($profilePayload, $idTokenClaims),
                ...
                'provider_avatar_url' => $this->nullableString(
                    data_get($profilePayload, 'picture') ?: data_get($idTokenClaims, 'picture')
                ),
            ],
            'tokens' => [
     ...
     * @param  array<string, mixed>  $idTokenClaims
     */
    private function resolveProviderEmailVerified(array $profilePayload, array $idTokenClaims): bool
    {
        $candidates = [
            data_get($profilePayload, 'email_verified'),
            data_get($idTokenClaims, 'email_verified'),
        ];
        ...
        // LinkedIn documents the email fields as optional on userinfo responses, so
        // when an email is present but no explicit flag is returned we treat it as usable.
        return $this->resolveProviderEmail($profilePayload, $idTokenClaims) !== '';
    }
}
