...
- Preuve externe : accepter uniquement un résultat JSON agrégé au schéma fermé v3. `manifest_hash` lie le scénario, `fixture_hash` la fixture v3, `baseline_fingerprint` le contexte complet, `target_origin_hash` l’origine approuvée sans l’exposer et `runner_hash` le harness exact. La fixture doit contenir une entrée pour chaque scénario prévu, avec sa stratégie signée et ses requêtes complètes ; aucune donnée sensible ne peut être versionnée. La cadence `request_interval_ms` et le délai `request_timeout_ms` font partie du profil vérifié. `attempted_requests` et `completed_requests` atteignent le `profile.minimum_completed_requests` ; tout champ brut ou inconnu est refusé.
