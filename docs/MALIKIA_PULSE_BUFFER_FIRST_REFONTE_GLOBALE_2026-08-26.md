...
Révision : 44 — compteur local durci, redaction d'erreur et protocole fournisseur préparés
...
La commande opérateur `php artisan pulse:buffer:inventory-legacy --json --confirm-read-only-scan` utilise par défaut la connexion de queue courante et la queue du workload `social_publish`. Pour une queue connue, locale ou ancienne, l’opérateur peut cibler un couple exact :
...
Chaque exécution inventorie uniquement :
...
- les jobs dont le `displayName` exact est `PublishSocialPostTargetJob` sur la queue `database` ciblée, avec séparation ready, delayed, réservation active et réservation expirée.

Les lectures du domaine s'effectuent dans une transaction et les références sont traitées par lots bornés. La queue est un relevé indépendant en un seul passage ; la sortie déclare explicitement qu'il n'existe pas d'atomicité inter-source. Une connexion ou une queue invalide est refusée avant le scan. Une table absente est non mesurable. Une queue `database` renommée peut être inspectée explicitement ; Redis, SQS et Beanstalkd sont seulement identifiés avec leur connexion, driver et nom de queue, puis déclarés non mesurables sans ouvrir leur driver. Leurs payloads et agrégats exigent donc toujours une preuve opérateur séparée. La commande ne découvre pas automatiquement les anciennes queues et n'interprète aucun snapshot JSON inconnu. Aucune valeur de ligne n'est exposée ; aucun credential ni identifiant distant n'est sélectionné ou exposé. Seuls des agrégats, des libellés plateforme/statut et des métadonnées techniques de capture/queue sont affichés. Seuls les candidats de queue préfiltrés par le nom de classe sont décodés et comptés comme illisibles. Chaque exécution, locale comprise, exige `--confirm-read-only-scan` : l'autorisation explicite ne dépend donc ni de `APP_ENV`, ni de l'option Artisan globale `--env`. L'opérateur ne doit la fournir que pour une base locale, un clone ou un environnement approuvé.
...
13. Créer de nouvelles lignes canal Buffer.
14. Faire valider le mapping par l’owner.
15. Exécuter une validation de capacités en shadow, sans publication.
16. Définir durée, seuils d’incident et critères du canary, puis signer le GO lancement pilote.
17. Choisir un tenant pilote et persister son transport Buffer par nouvelle cible.
18. Publier un scénario contrôlé.
19. Réconcilier et observer.
20. Drainer ou annuler/recréer explicitement les posts legacy delayed.
21. Étendre le canary puis réunir les preuves du GO général.
22. Fermer la création de connexions directes.
23. Découpler le service de suppression de données Facebook du transport Pulse.
...
- un design de première migration limité aux champs de transport legacy qui seront consommés immédiatement par les sélecteurs et le worker existants ;
...
La confirmation propriétaire/payeur, le runner MySQL réel, la revue du design/backfill et l’outil de ciblage explicite des queues sont acquis. Ce gate pourra être accordé sans fermer les autres preuves commerciales de `BUF-P0-10`, car il ne couvre que la préparation locale et réversible du schéma direct. Il n'est pas encore accordé : le clone représentatif, la liste exhaustive et les preuves effectives des queues externes ou anciennes, ainsi que la qualification de leurs anomalies, restent à produire. Les tests `up → down → up` et du backfill sur SQLite/MySQL seront exécutés seulement après ce GO, lorsque la migration existera.
...
**GO_WP2A_CONTRACT_FAKE_LOCAL_ONLY · WP2A_LOCAL_VALIDATED · ACCOUNT_OWNER_PAYER_CONFIRMED · BUFFER_COMMERCIAL_BASELINE_DOCUMENTED · BUFFER_LOGICAL_REQUEST_EVIDENCE_INSTRUMENTED · PRE_WP2B_INVENTORY_LOCAL_VALIDATED · PRE_WP2B_QUEUE_SCOPE_TOOLING_VALIDATED · MYSQL_COMPATIBILITY_GATE_VALIDATED · P0_GATES_OPEN · WP2B_SCHEMA_GATE_PENDING · NO_GO_BUFFER_RUNTIME · NO_GO_BUFFER_PILOT · NO_GO_BUFFER_PRODUCTION** : le contrat/fake, l'outil d'inventaire local et son exécution automatisée sur un vrai MySQL sont validés. Chaque client possède et paie son compte Buffer ; les prix officiels, les prérequis Facebook et la recommandation `Essentials` sont documentés. Le harnais expose maintenant son budget logique par phase et le protocole de question fournisseur est prêt, tandis que le quota OAuth multi-client, la consommation runtime réelle et le parcours pilote restent ouverts. Aucun `GO_WP2B_SCHEMA_LOCAL_ONLY` n'est encore inscrit : il manque le clone représentatif et les queues externes ou anciennes. `NO_GO_BUFFER_RUNTIME` vise WP2-C/WP3/WP4 et ne révoque pas les harnais WP1 soumis à leurs propres autorisations. Les gates P0 restent ouverts selon leur portée.
