...
Révision : 51 — gate PHP global stabilisé quelle que soit la locale numérique
...
Le contrat `pulse_legacy_inventory_v2` remplace le ciblage unitaire v1. Sans `--queue-scope`, la commande utilise toujours la connexion courante et la queue du workload `social_publish`, mais seulement pour un relevé exploratoire dont la liste n'est pas attestée. Une preuve candidate pour le gate doit déclarer chaque couple `connexion:queue`, le contexte de source et l'attestation séparée de complétude :
...
- les jobs dont le `displayName` exact est `PublishSocialPostTargetJob` sur chaque queue `database` déclarée, avec séparation ready, delayed, réservation active et réservation expirée ;
- les mêmes jobs exacts encore présents dans le stockage `failed_jobs`, afin qu'une future commande `queue:retry` ne réactive pas silencieusement un routage legacy.
...
| Queue `social-publish` | 0 job exact, 0 candidat illisible |
| `failed_jobs` | 0 job social exact, 0 candidat illisible |
| Manifeste | source `local`, 1 scope sur 1 mesurable, liste non attestée, capture séquentielle non atomique |

Ce relevé prouve le fonctionnement manuel sur la base MySQL locale courante. Le manifeste v2 passe 23 tests / 246 assertions et PHPStan 913/913. Ces preuves ne décrivent toujours ni l'état d'un clone représentatif ni celui de la production. Avant WP2-B schéma, il reste à exécuter ce format sur le clone choisi avec la liste attestée, produire les preuves séparées de toute queue non database, décider du sort de chaque job social échoué, qualifier les anomalies, définir la dérivation et l'immutabilité de `logical_destination_key`, puis approuver un gate local borné pour les migrations/backfills.
...
La preuve conservée sera uniquement la date, une référence de ticket, la réponse écrite expurgée et la décision de capacité qui en découle. Si Buffer exige le `client_id` public pour instruire la demande, son partage devra être autorisé explicitement sans jamais joindre le `client_secret`. Envoyer ce courriel constitue une communication externe et demandera l'autorisation explicite de Jules ; sa préparation ne bloque aucun travail local.
...
La confirmation propriétaire/payeur, le runner MySQL réel, la revue initiale du design/backfill et le manifeste multi-scopes v2 sont acquis. Ce gate pourra être accordé sans fermer les autres preuves commerciales de `BUF-P0-10`, car il ne couvre que la préparation locale et réversible du schéma direct. Il n'est pas encore accordé : le clone représentatif, la liste attestée, les preuves effectives des queues externes ou anciennes, la qualification des `failed_jobs` et anomalies, ainsi que le contrat final de `logical_destination_key`, restent à produire. Les tests `up → down → up` et du backfill sur SQLite/MySQL seront exécutés seulement après ce GO, lorsque la migration existera.
...
**GO_WP2A_CONTRACT_FAKE_LOCAL_ONLY · WP2A_LOCAL_VALIDATED · ACCOUNT_OWNER_PAYER_CONFIRMED · BUFFER_COMMERCIAL_BASELINE_DOCUMENTED · BUFFER_LOGICAL_REQUEST_EVIDENCE_INSTRUMENTED · PRE_WP2B_INVENTORY_LOCAL_VALIDATED · PRE_WP2B_QUEUE_SCOPE_TOOLING_VALIDATED · PRE_WP2B_MULTI_SCOPE_MANIFEST_LOCAL_VALIDATED · MYSQL_COMPATIBILITY_GATE_VALIDATED · PR140_CI_VALIDATED · P0_GATES_OPEN · WP2B_SCHEMA_GATE_PENDING · NO_GO_BUFFER_RUNTIME · NO_GO_BUFFER_PILOT · NO_GO_BUFFER_PRODUCTION** : le contrat/fake, le manifeste d'inventaire v2 et son exécution exploratoire sur un vrai MySQL local sont validés. Chaque client possède et paie son compte Buffer ; les prix officiels, les prérequis Facebook et la recommandation `Essentials` sont documentés. Le harnais expose son budget logique par phase et le protocole de question fournisseur est prêt, tandis que le quota OAuth multi-client, la consommation runtime réelle et le parcours pilote restent ouverts. Aucun `GO_WP2B_SCHEMA_LOCAL_ONLY` n'est encore inscrit : il manque le clone représentatif, les preuves des queues externes ou anciennes, la qualification des failed jobs/anomalies et le contrat final de destination logique. `NO_GO_BUFFER_RUNTIME` vise WP2-C/WP3/WP4 et ne révoque pas les harnais WP1 soumis à leurs propres autorisations. Les gates P0 restent ouverts selon leur portée.
