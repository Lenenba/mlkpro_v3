...
Révision : 55 — Étape 1 : checkpoint local intégré entièrement vert
...
- les agrégats des connexions et cibles sociales ;
...
Ce relevé prouve le fonctionnement manuel sur la base MySQL locale courante. Le manifeste v2 étendu passe 29 tests / 360 assertions sur SQLite et MySQL, ainsi que PHPStan sur 913 fichiers ; la base MySQL isolée de test a été supprimée et son absence vérifiée. Ces preuves ne décrivent toujours ni l'état d'un clone représentatif ni celui de la production. Avant WP2-B schéma, il reste à exécuter ce format sur le clone choisi avec la liste attestée, recouper les anciens noms de queues et de classes avec les sources d'exploitation, produire les preuves séparées de toute queue non database, décider du sort de chaque job Pulse échoué, qualifier les anomalies, finaliser le plan de backfill/rollback à partir de ces faits, puis approuver le gate local borné pour les migrations/backfills. Le contrat de `logical_destination_key`, lui, est déjà figé ci-dessous.
...
- pour une connexion legacy, la préimage est le tableau de chaînes `['malikia-pulse-logical-destination', 'v1', tenant, platform, external_id]` : `tenant` est obligatoirement une chaîne décimale canonique, `platform=strtolower(trim(platform))` doit être autorisée et `external_id=trim(external_account_id)` doit être non vide, de 191 caractères maximum, UTF-8 valide et sans caractère de contrôle ; la casse et les octets internes de l'identifiant opaque sont préservés ;
