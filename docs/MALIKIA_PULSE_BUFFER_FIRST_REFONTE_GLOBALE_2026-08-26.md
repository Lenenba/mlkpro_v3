...
Révision : 59 — Étape 1 : revalidation locale terminée, frontière H1 explicitée
...
Statut de livraison : **WP0/WP0-S validés localement — gate d’indexation vert — gate de déploiement ouvert — WP1-A/WP1-B/WP1-D authentifiés en lecture seule — WP1-H prouve réellement create, edit et delete sur un brouillon Facebook standard — WP1-I qualifie `move@draft` comme frontière négative provisoire — le harnais expose le nombre d’opérations GraphQL logiques tentées sans le présenter comme une consommation fournisseur — WP2-A contract/fake est validé localement — la PR #140 passe ses validations Laravel, MySQL et navigateur sur son dernier checkpoint confirmé — l’inventaire pré-WP2-B v2 agrège des scopes de queue explicites, les jobs sociaux échoués, la provenance opérateur, la fenêtre de capture, la préparation des identités logiques et la topologie Pulse issue de la configuration Laravel effective sans ouvrir les drivers externes ni exposer leurs identifiants, leurs clés, les noms de profils ou les noms de queues dans cette nouvelle projection — le canonicaliseur de `logical_destination_key` est centralisé, consommé par le préflight read-only et validé localement sur SQLite et MySQL — la preuve configurée reste explicitement distincte des processus réellement déployés — le gate PHP global du checkpoint précédent passe sous locale numérique à virgule sans modifier le comportement Pulse — le pilotage courant comporte exactement trois macro-étapes ; l’Étape 1 regroupe la preuve représentative puis, après son GO unique, la fondation WP2-B additive et réversible — l’exécution locale reste exploratoire et ne remplace pas le clone représentatif — cette mise en route n’accorde ni `GO_WP2B_SCHEMA_LOCAL_ONLY`, ni migration, ni backfill, ni runtime Buffer — tous les gates BUF-P0 restent ouverts pour le runtime Buffer, le pilote et la production selon leur portée**

Statut de décision : **RISK_PROPORTIONATE_VALIDATION_ACCEPTED · THREE_MACRO_STEPS_ACCEPTED · REPRESENTATIVE_CLONE_STRATEGY_ACCEPTED · BUFFER_SUPPORT_CONTACT_SENT · MACRO_STEP_1_IN_PROGRESS · GO_WP2A_CONTRACT_FAKE_LOCAL_ONLY · WP2A_LOCAL_VALIDATED · ACCOUNT_OWNER_PAYER_CONFIRMED · BUFFER_COMMERCIAL_BASELINE_DOCUMENTED · BUFFER_LOGICAL_REQUEST_EVIDENCE_INSTRUMENTED · LOGICAL_DESTINATION_KEY_CONTRACT_FROZEN · LOGICAL_DESTINATION_KEY_CANONICALIZER_LOCAL_VALIDATED · PRE_WP2B_IDENTITY_READINESS_LOCAL_VALIDATED · PULSE_CONFIGURED_TOPOLOGY_LOCAL_VALIDATED · PRE_WP2B_INVENTORY_LOCAL_VALIDATED · PRE_WP2B_PULSE_JOB_INVENTORY_LOCAL_VALIDATED · PRE_WP2B_QUEUE_SCOPE_TOOLING_VALIDATED · PRE_WP2B_MULTI_SCOPE_MANIFEST_LOCAL_VALIDATED · PRE_WP2B_MULTI_SCOPE_MATRIX_VALIDATED · MYSQL_COMPATIBILITY_GATE_VALIDATED · PR140_CI_VALIDATED · P0_GATES_OPEN · WP2B_SCHEMA_GATE_PENDING · NO_GO_BUFFER_RUNTIME · NO_GO_BUFFER_PILOT · NO_GO_BUFFER_PRODUCTION**
...
| --- | --- | --- | --- | --- | --- |
| **Étape 1 — Fondation représentative et WP2-B additive** | Pré-WP2-B et WP2-B : les données legacy, les queues, l'identité logique, le schéma et le backfill forment une seule fondation réversible | **EN COURS** | `WP2B_SCHEMA_LOCAL_GREEN=true` : clone représentatif scanné ; liste complète des queues attestée ; preuves externes attachées ; chaque job/anomalie couvert ; contrat final figé ; migrations additives, backfill `direct` / `direct_v1`, cycles SQLite/MySQL, isolation, immutabilité et rollback verts ; aucun binding ou trafic Buffer | Client HTTP Buffer, OAuth/canal Buffer, outbox distante, UX, pilote, cutover et suppression du direct | L'opérateur fournit l'attestation de source/liste comme preuve. Jules rend l'unique validation humaine de l'étape sur le dossier représentatif : `GO_WP2B_SCHEMA_LOCAL_ONLY` ou NO-GO. Après ce GO, les gates automatisés déterminent la sortie |
| **Étape 2 — Runtime Facebook fiable et UX, désactivés par défaut** | Reste de WP1, WP2-C à WP5 et découplage Facebook Data Deletion : contrat fournisseur, gateway, outbox, réconciliation, UX et observabilité se valident comme un produit activable | **EN ATTENTE** | P0 applicables fermés ; isolation, idempotence et single-transport prouvées ; runtime, UX, accessibilité et observabilité verts ; tous les flags de livraison/pilote restent désactivés par défaut | Trafic pilote, canary, mapping/cutover, drain, retrait du direct et social login utilisateur | Jules rend l'unique validation humaine de l'étape sur le dossier de lancement et accorde ou refuse le GO pilote |
| **Étape 3 — Pilote, cutover et retrait prouvé du direct** | WP6 et WP7 : mapping owner-validé, pilote, canary, rollback, drain et suppression du legacy forment une seule séquence opérationnelle | **NO-GO** | Canary conforme aux seuils prévalidés ; rollback prouvé ; aucune référence active, cible ou job legacy risqué ; drain terminé ; code/config/secrets directs retirés sans toucher au social login | Nouvelles promesses analytics, autre canal que Facebook et toute réécriture destructive des migrations appliquées | Jules rend l'unique validation humaine de l'étape : GO général et retrait du direct, ou NO-GO, sur les preuves du canary et du drain |

`MACRO_STEP_1_IN_PROGRESS` ne signifie pas `GO_WP2B_SCHEMA_LOCAL_ONLY` : aucune migration ne démarre avant la validation humaine unique de l’Étape 1. Après ce GO borné, la fondation reste dans la même macro-étape et ne sort que lorsque `WP2B_SCHEMA_LOCAL_GREEN=true` est prouvé par les gates automatisés.
...
| BUF-P0-03 | Webhook de statut disponible ou prévu | Contrat ou réponse écrite Buffer, ou décision explicite de polling | Backend | WP2-C/runtime Buffer, réconciliation WP4 distante et pilote | Ouvert |
...
| BUF-P0-10 | Modèle commercial compte client | Propriétaire/payeur, parcours, prix/plan, capacité et prérequis validés | Produit | WP2-B schéma jusqu'au GO local séparé, WP2-C/runtime Buffer, pilote, production et généralisation | Partiellement fermé — propriétaire/payeur confirmé ; prix officiels, prérequis et recommandation `Essentials` documentés ; compteur logique WP1 et protocole fournisseur préparés ; quota OAuth multi-client, consommation runtime réelle, acceptation du plan et parcours pilote ouverts |
...
Ce relevé prouve le fonctionnement manuel sur la base MySQL locale courante. Le canonicaliseur passe 27 tests / 53 assertions ; l'inventaire v2 passe 33 tests / 402 assertions sur SQLite et la matrice combinée passe 60 tests / 455 assertions sur MySQL. PHPStan passe globalement sur 914 fichiers ; la base MySQL isolée de test a été supprimée et son absence vérifiée. Ces preuves ne décrivent toujours ni l'état d'un clone représentatif ni celui de la production. Avant WP2-B schéma, il reste à exécuter ce format sur le clone choisi avec la liste attestée, recouper les noms de queues et de classes déjà fermés dans l'historique suivi avec les overrides et sources d'exploitation externes, produire les preuves séparées de toute queue non database, décider du sort de chaque job Pulse échoué, qualifier les anomalies, finaliser le plan de backfill/rollback à partir de ces faits, puis approuver le gate local borné pour les migrations/backfills. Le contrat de `logical_destination_key`, lui, est déjà figé et désormais exécutable ci-dessous.
...
9. Figer les identités de routage déjà en queue, garder leurs credentials rafraîchissables et conserver leur worker direct pendant le futur drain.
...
14. Créer de nouvelles lignes canal Buffer.
15. Faire valider le mapping par l’owner.
16. Exécuter une validation de capacités en shadow, sans publication.
17. Définir durée, seuils d’incident et critères du canary, puis signer le GO lancement pilote.
...
- l'inventaire explicite de toute queue non database, des anciennes queues susceptibles de contenir un job social delayed et des `failed_jobs` sociaux encore rejouables, avec politique de drain, archivage ou interdiction de retry ;
...
La confirmation propriétaire/payeur, le runner MySQL réel, le contrat final de `logical_destination_key`, l'historique Git suivi des classes/queues et le manifeste multi-scopes v2 couvrant les deux workloads Pulse connus sont acquis. Ce gate pourra être accordé sans fermer les autres preuves commerciales de `BUF-P0-10`, car il ne couvre que la préparation locale et réversible du schéma direct. Il n'est pas encore accordé : le clone représentatif, la liste attestée recoupée avec les processus et la topologie réellement déployés ainsi que les overrides d'environnement présents ou passés, les preuves effectives des queues externes ou anciennes, la qualification des `failed_jobs` et anomalies, ainsi que le plan final de backfill/rollback ajusté à ces faits restent à produire. Les tests `up → down → up` et du backfill sur SQLite/MySQL seront exécutés seulement après ce GO, lorsque la migration existera.
...
**RISK_PROPORTIONATE_VALIDATION_ACCEPTED · THREE_MACRO_STEPS_ACCEPTED · REPRESENTATIVE_CLONE_STRATEGY_ACCEPTED · BUFFER_SUPPORT_CONTACT_SENT · MACRO_STEP_1_IN_PROGRESS · GO_WP2A_CONTRACT_FAKE_LOCAL_ONLY · WP2A_LOCAL_VALIDATED · ACCOUNT_OWNER_PAYER_CONFIRMED · BUFFER_COMMERCIAL_BASELINE_DOCUMENTED · BUFFER_LOGICAL_REQUEST_EVIDENCE_INSTRUMENTED · LOGICAL_DESTINATION_KEY_CONTRACT_FROZEN · LOGICAL_DESTINATION_KEY_CANONICALIZER_LOCAL_VALIDATED · PRE_WP2B_IDENTITY_READINESS_LOCAL_VALIDATED · PULSE_CONFIGURED_TOPOLOGY_LOCAL_VALIDATED · PRE_WP2B_INVENTORY_LOCAL_VALIDATED · PRE_WP2B_PULSE_JOB_INVENTORY_LOCAL_VALIDATED · PRE_WP2B_QUEUE_SCOPE_TOOLING_VALIDATED · PRE_WP2B_MULTI_SCOPE_MANIFEST_LOCAL_VALIDATED · PRE_WP2B_MULTI_SCOPE_MATRIX_VALIDATED · MYSQL_COMPATIBILITY_GATE_VALIDATED · PR140_CI_VALIDATED · P0_GATES_OPEN · WP2B_SCHEMA_GATE_PENDING · NO_GO_BUFFER_RUNTIME · NO_GO_BUFFER_PILOT · NO_GO_BUFFER_PRODUCTION** : le contrat/fake, le canonicaliseur de destination logique et le manifeste d'inventaire v2 couvrant les deux workloads Pulse connus sont validés localement. Le préflight agrégé confirme aussi sur la base locale que l'unique connexion est dérivable sans échec ni groupe doublon/collision, mais n'émet aucune clé et ne remplace pas le clone représentatif. Chaque client possède et paie son compte Buffer ; les prix officiels, les prérequis Facebook et la recommandation `Essentials` sont documentés. La question fournisseur sur le quota OAuth multi-client a été envoyée sans secret ; sa réponse, la consommation runtime réelle et le parcours pilote restent ouverts. L’Étape 1/3 est en cours sur la preuve représentative. La stratégie de clone est acceptée, mais le clone n’est pas déclaré inventorié. Aucun `GO_WP2B_SCHEMA_LOCAL_ONLY` n'est encore inscrit : il manque le clone représentatif, les processus et la topologie réellement déployés ainsi que les overrides d'environnement présents ou passés, les preuves des queues externes ou anciennes, la qualification des `failed_jobs`/anomalies et le plan final de backfill/rollback. `NO_GO_BUFFER_RUNTIME` vise WP2-C/WP3/WP4 et ne révoque pas les harnais WP1 soumis à leurs propres autorisations. Les gates P0 restent ouverts selon leur portée.
