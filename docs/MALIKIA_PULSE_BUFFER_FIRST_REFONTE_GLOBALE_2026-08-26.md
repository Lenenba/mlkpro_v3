...
Révision : 25 — checkpoint WP1-E publié et vérifié
...
Statut de livraison : **WP0/WP0-S validés localement — gate d’indexation vert — gate de déploiement ouvert — WP1-A/WP1-B/WP1-D authentifiés en lecture seule — deux create WP1-C refusés sans objet distant — contrat WP1-E préparé, validé et publié sur la branche feature ; causalité metadata non prouvée et aucun troisième essai exécuté ou autorisé**
...
Le seul candidat minimal cohérent avec un brouillon Facebook texte est donc `metadata.facebook.type=post`. Cette conclusion est une **validation de forme**, pas une preuve causale : le schéma autorise aussi l'omission complète de `metadata`, et l'erreur `InvalidInputError` ne fournit aucun chemin de champ structuré. Un futur essai exigerait une nouvelle tranche explicitement autorisée, des variables strictement limitées à cette metadata et des postconditions sur le metadata Facebook retourné. Aucun essai supplémentaire n'est inclus dans WP1-D.
