...
Révision : 58 — Étape 1 : identité logique et topologie configurée inventoriées localement
...
Branche distante : `origin/feature/pulse-buffer-refonte`, checkpoint fonctionnel précédent publié `0f6f04c4` ; dernier checkpoint CI confirmé `42c2ef94` via quality #412 ; la validation distante du lot courant après publication est laissée à Jules
...
La confirmation propriétaire/payeur, le runner MySQL réel, le contrat final de `logical_destination_key` et le manifeste multi-scopes v2 couvrant les deux workloads Pulse connus sont acquis. Ce gate pourra être accordé sans fermer les autres preuves commerciales de `BUF-P0-10`, car il ne couvre que la préparation locale et réversible du schéma direct. Il n'est pas encore accordé : le clone représentatif, la liste attestée recoupée avec la topologie déployée et l'historique des renommages, les preuves effectives des queues externes ou anciennes, la qualification des `failed_jobs` et anomalies, ainsi que le plan final de backfill/rollback ajusté à ces faits restent à produire. Les tests `up → down → up` et du backfill sur SQLite/MySQL seront exécutés seulement après ce GO, lorsque la migration existera.
