...
Révision : 57 — Étape 1 : identité logique exécutable et inventoriée localement
...
Ce relevé prouve le fonctionnement manuel sur la base MySQL locale courante. Le canonicaliseur passe 27 tests / 53 assertions ; l'inventaire v2 passe 32 tests / 381 assertions sur SQLite et la matrice combinée passe 59 tests / 434 assertions sur MySQL. PHPStan passe globalement sur 914 fichiers ; la base MySQL isolée de test a été supprimée et son absence vérifiée. Ces preuves ne décrivent toujours ni l'état d'un clone représentatif ni celui de la production. Avant WP2-B schéma, il reste à exécuter ce format sur le clone choisi avec la liste attestée, recouper les anciens noms de queues et de classes avec les sources d'exploitation, produire les preuves séparées de toute queue non database, décider du sort de chaque job Pulse échoué, qualifier les anomalies, finaliser le plan de backfill/rollback à partir de ces faits, puis approuver le gate local borné pour les migrations/backfills. Le contrat de `logical_destination_key`, lui, est déjà figé et désormais exécutable ci-dessous.
