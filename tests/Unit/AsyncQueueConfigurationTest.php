...
test('async workloads apply explicit queue names to jobs and notifications', function () {
    ...
    config()->set('async.workloads.reservation_reconciliation.queue', 'reservation-reconciliation-test');
...
test('async workloads expose configured backoff policies', function () {
    ...
    config()->set('async.workloads.reservation_reconciliation.backoff', [60, 300, 900]);
...
test('expired walk-in cleanup is unique and serialized per tenant', function () {
    $job = new CloseExpiredWalkInsForAccountJob(92);
    $middleware = $job->middleware();

    expect($job->uniqueId())->toBe('92')
        ...
        ->and($middleware[0]->key)->toBe('reservation-walk-in-close:92')
        ->and($middleware[0]->releaseAfter)->toBe(60)
        ->and($middleware[0]->expiresAfter)->toBe(180)
        ->and($middleware[0]->shareKey)->toBeTrue();
});

test('reservation reconciliation is unique and serialized per tenant', function () {
    $job = new ReconcilePastReservationsForAccountJob(91);
    $middleware = $job->middleware();

    expect($job->uniqueId())->toBe('91')
        ->and($job->uniqueFor)->toBe(900)
        ->and($job->tries)->toBe(3)
        ->and($job->timeout)->toBe(120)
        ->and($job->failOnTimeout)->toBeTrue()
        ->and($middleware)->toHaveCount(1)
        ->and($middleware[0])->toBeInstanceOf(WithoutOverlapping::class)
        ->and($middleware[0]->key)->toBe('reservation-reconciliation:91')
        ->and($middleware[0]->releaseAfter)->toBe(60)
        ->and($middleware[0]->expiresAfter)->toBe(180)
        ->and($middleware[0]->shareKey)->toBeTrue();
});
